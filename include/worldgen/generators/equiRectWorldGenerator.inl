...
thread_local ThreadStorage EquiRectWorldGenerator<_Region, _Chunk>::m_threadStorage;

//template<typename _Region, typename _Chunk>
//std::vector<float> EquiRectWorldGenerator<_Region, _Chunk>::heightMap;
//template<typename _Region, typename _Chunk>
//std::vector<float> EquiRectWorldGenerator<_Region, _Chunk>::blockHeightMap;
//template<typename _Region, typename _Chunk>
//std::vector<float> EquiRectWorldGenerator<_Region, _Chunk>::blockScaleMap;
//template<typename _Region, typename _Chunk>
//std::vector<float> EquiRectWorldGenerator<_Region, _Chunk>::xMap;
//template<typename _Region, typename _Chunk>
//std::vector<float> EquiRectWorldGenerator<_Region, _Chunk>::yMap;
//template<typename _Region, typename _Chunk>
//std::vector<float> EquiRectWorldGenerator<_Region, _Chunk>::zMap;
//template<typename _Region, typename _Chunk>
//std::vector<float> EquiRectWorldGenerator<_Region, _Chunk>::layerMap;
//template<typename _Region, typename _Chunk>
//std::unique_ptr<HastyNoise::VectorSet> EquiRectWorldGenerator<_Region, _Chunk>::vectorSet;
//
//template<typename _Region, typename _Chunk>
//std::vector<float> EquiRectWorldGenerator<_Region, _Chunk>::regionHeightMap;
//template<typename _Region, typename _Chunk>
//std::unique_ptr<HastyNoise::VectorSet> EquiRectWorldGenerator<_Region, _Chunk>::regionVectorSet;

template<typename _Region, typename _Chunk>
EquiRectWorldGenerator<_Region, _Chunk>::EquiRectWorldGenerator()
{
    m_plateCount=16;
...
template<typename _Region, typename _Chunk>
void EquiRectWorldGenerator<_Region, _Chunk>::create(WorldDescriptors *descriptors, Progress &progress)
{
...
template<typename _Region, typename _Chunk>
void EquiRectWorldGenerator<_Region, _Chunk>::initialize(WorldDescriptors *descriptors)
{
...
//    m_cellularNoise->SetNoiseType(HastyNoise::NoiseType::Cellular);
    ...
    m_os2Noise=FastNoise::New<FastNoise::OpenSimplex2>();

    m_domainScale=FastNoise::New<FastNoise::DomainScale>();
    m_domainWarp=FastNoise::New<FastNoise::DomainWarpGradient>();
    m_domainWarpFractal=FastNoise::New<FastNoise::DomainWarpFractalIndependant>();
    m_fractalFbmNoise=FastNoise::New<FastNoise::FractalFBm>();

    m_cellularNoise=FastNoise::New<FastNoise::CellularValue>();
    m_cellularDistanceNoise=FastNoise::New<FastNoise::CellularDistance>();

    FastSIMD::eLevel simdLevel=m_cellularNoise->GetSIMDLevel();
...
template<typename _Region, typename _Chunk>
void EquiRectWorldGenerator<_Region, _Chunk>::loadDescriptors(WorldDescriptors *descriptors)
{
    initialize(descriptors);
}

template<typename _Region, typename _Chunk>
void EquiRectWorldGenerator<_Region, _Chunk>::saveDescriptors(WorldDescriptors *descriptors)
{
...
template<typename _Region, typename _Chunk>
void EquiRectWorldGenerator<_Region, _Chunk>::saveDescriptors(std::string &descriptors)
{
    size_t size=1024;
...
bool EquiRectWorldGenerator<_Region, _Chunk>::load(WorldDescriptors *descriptors, const std::string &directory, Progress &progress)
{
...
template<typename _Region, typename _Chunk>
template<typename _FileIO>
void EquiRectWorldGenerator<_Region, _Chunk>::save(const std::string &directory)
{
    typedef generic::io::fs<_FileIO> fs;
...
template<typename _Region, typename _Chunk>
void EquiRectWorldGenerator<_Region, _Chunk>::generateWorldOverview(Progress &progress)
{
...
template<typename _Region, typename _Chunk>
template<typename _FileIO>
bool EquiRectWorldGenerator<_Region, _Chunk>::loadWorldOverview(const std::string &fileName)
{
    typedef generic::io::fs<_FileIO> fs;
...
template<typename _Region, typename _Chunk>
template<typename _FileIO>
void EquiRectWorldGenerator<_Region, _Chunk>::saveWorldOverview(const std::string &fileName)
{
    typedef generic::io::fs<_FileIO> fs;
...
template<typename _Region, typename _Chunk>
template<typename _FileIO>
bool EquiRectWorldGenerator<_Region, _Chunk>::loadNormalize(const std::string &fileName)
{
    typedef generic::io::fs<_FileIO> fs;
...
template<typename _Region, typename _Chunk>
template<typename _FileIO>
void EquiRectWorldGenerator<_Region, _Chunk>::saveNormalize(const std::string &fileName)
{
    typedef generic::io::fs<_FileIO> fs;
...
template<typename _Region, typename _Chunk>
void EquiRectWorldGenerator<_Region, _Chunk>::generatePlates(Progress &progress)
{
...
//height map
    m_domainScale->SetSource(m_os2Noise);
    m_fractalFbmNoise->SetSource(m_domainScale);

    m_domainScale->SetScale(0.01f);
    m_fractalFbmNoise->SetGain(0.5f);
    m_fractalFbmNoise->SetOctaveCount(4);
    m_fractalFbmNoise->SetLacunarity(2.0f);
    m_fractalFbmNoise->GenPositionArray3D(heightMap.data(), influenceMapSize, m_xPositions.data(), m_yPositions.data(), m_zPositions.data(),
        0.0f, 0.0f, 0.0f, m_plateSeed);

    progress.update("Generating terrain", 20, false);

    m_domainScale->SetScale(0.05f);
    m_fractalFbmNoise->SetGain(0.5f);
    m_fractalFbmNoise->SetOctaveCount(4);
    m_fractalFbmNoise->SetLacunarity(2.0f);
    m_fractalFbmNoise->GenPositionArray3D(terrainScaleMap.data(), influenceMapSize, m_xPositions.data(), m_yPositions.data(), m_zPositions.data(),
        0.0f, 0.0f, 0.0f, m_plateSeed);

    progress.update("Generating air currents", 25, false);

    m_domainWarp->SetSource(m_domainScale);
    m_fractalFbmNoise->SetSource(m_domainWarp);

    m_domainScale->SetScale(0.01f);
    m_domainWarp->SetWarpAmplitude(0.5f);
    m_domainWarp->SetWarpFrequency(1.0f);
    m_fractalFbmNoise->SetGain(0.5f);
    m_fractalFbmNoise->SetOctaveCount(4);
    m_fractalFbmNoise->SetLacunarity(2.0f);
    m_fractalFbmNoise->GenPositionArray3D(nsAirCurrent.data(), influenceMapSize, m_xPositions.data(), m_yPositions.data(), m_zPositions.data(),
...
//    m_continentPerlin->FillSet(ewAirCurrent.data(), m_influenceVectorSet.get());
    m_fractalFbmNoise->GenPositionArray3D(ewAirCurrent.data(), influenceMapSize, m_xPositions.data(), m_yPositions.data(), m_zPositions.data(),
        0.0f, 0.0f, 0.0f, m_plateSeed+4);

    progress.update("Generating tectonic plates", 30, false);

//plate map
    m_cellularNoise->SetDistanceFunction(FastNoise::DistanceFunction::Hybrid);
    m_cellularNoise->SetValueIndex(0);
    m_domainWarp->SetSource(m_cellularNoise);
    m_domainWarp->SetWarpAmplitude(0.5f);
    m_domainWarp->SetWarpFrequency(1.0f);
    m_domainWarpFractal->SetSource(m_domainWarp);
    m_domainWarpFractal->SetGain(0.5f);
    m_domainWarpFractal->SetOctaveCount(5);
    m_domainWarpFractal->SetLacunarity(2.0f);
    m_domainScale->SetSource(m_domainWarpFractal);
    m_domainScale->SetScale(m_descriptorValues.m_plateFrequency);

    m_domainScale->GenPositionArray3D(plateMap.data(), influenceMapSize, m_xPositions.data(), m_yPositions.data(), m_zPositions.data(),
        0.0f, 0.0f, 0.0f, m_plateSeed);
//    m_cellularNoise->GenPositionArray3D(plateMap.data(), influenceMapSize, m_xPositions.data(), m_yPositions.data(), m_zPositions.data(),
//        0.0f, 0.0f, 0.0f, m_plateSeed);
//    m_cellularNoise->FillSet(plateMap.data(), m_influenceVectorSet.get());

//Border plate Map
    m_cellularNoise->SetValueIndex(1);
    m_domainScale->GenPositionArray3D(plate2Map.data(), influenceMapSize, m_xPositions.data(), m_yPositions.data(), m_zPositions.data(),
        0.0f, 0.0f, 0.0f, m_plateSeed);
...
//Distance Map
    ...
    m_cellularDistanceNoise->SetDistanceFunction(FastNoise::DistanceFunction::Hybrid);
    m_cellularDistanceNoise->SetReturnType(CellularReturnType::Index0Div1);//gives distance to border
    m_cellularDistanceNoise->SetDistanceIndex0(0);
    m_cellularDistanceNoise->SetDistanceIndex1(1);
//    m_cellularDistanceNoise->FillSet(plateDistanceMap.data(), m_influenceVectorSet.get());
//    m_domainScale->SetSource(m_cellularDistanceNoise);
    m_domainWarp->SetSource(m_cellularDistanceNoise);

    m_domainScale->GenPositionArray3D(plateDistanceMap.data(), influenceMapSize, m_xPositions.data(), m_yPositions.data(), m_zPositions.data(),
        0.0f, 0.0f, 0.0f, m_plateSeed);
...
//continent map
    m_cellularNoise->SetValueIndex(0);
    m_domainWarp->SetSource(m_cellularNoise);
    m_domainScale->SetScale(m_descriptorValues.m_continentFrequency);
//    m_cellularDistanceNoise->SetSeed(m_continentSeed);
//    m_cellularDistanceNoise->SetFrequency(m_descriptorValues.m_continentFrequency);
//    m_cellularDistanceNoise->FillSet(continentMap.data(), m_influenceVectorSet.get());
    m_domainScale->GenPositionArray3D(continentMap.data(), influenceMapSize, m_xPositions.data(), m_yPositions.data(), m_zPositions.data(),
...
template<typename _Region, typename _Chunk>
void EquiRectWorldGenerator<_Region, _Chunk>::generateContinents(Progress &progress)
{
...
template<typename _Region, typename _Chunk>
unsigned int EquiRectWorldGenerator<_Region, _Chunk>::generateChunk(const glm::vec3 &startPos, const glm::ivec3 &chunkSize, void *buffer, size_t bufferSize, size_t lod)
{
...
template<typename _Region, typename _Chunk>
unsigned int EquiRectWorldGenerator<_Region, _Chunk>::generateRegion(const glm::vec3 &startPos, const glm::ivec3 &regionSize, void *buffer, size_t bufferSize, size_t lod)
{
...
template<typename _Region, typename _Chunk>
int EquiRectWorldGenerator<_Region, _Chunk>::getBaseHeight(const glm::vec2 &pos)
{
...
//template<typename _Region, typename _Chunk>
//unsigned int EquiRectWorldGenerator<_Region, _Chunk>::generateHeightMap(const glm::vec3 &startPos, const glm::ivec3 &regionSize, void *buffer, size_t bufferSize, size_t lod)
//{
//    glm::vec3 scaledOffset=startPos*m_descriptorValues.m_noiseScale;
//    glm::vec3 position=scaledOffset;
//
...
template<typename _Region, typename _Chunk>
void EquiRectWorldGenerator<_Region, _Chunk>::buildHeightMap(const glm::vec3 &startPos, const glm::ivec3 &lodSize, size_t stride)
{
    int heightMapSize=HastyNoise::AlignedSize(lodSize.x*lodSize.y, m_simdLevel);
...
//template<typename _Region, typename _Chunk>
//void EquiRectWorldGenerator<_Region, _Chunk>::generate()
//{
//
//}
...
template<typename _Region, typename _Chunk>
void EquiRectWorldGenerator<_Region, _Chunk>::updateInfluenceNeighbors()
{
